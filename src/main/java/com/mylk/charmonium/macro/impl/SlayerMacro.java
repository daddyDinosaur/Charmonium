...
public class SlayerMacro extends AbstractMacro {
   ...
   private static SlayerMacro instance;

   public static SlayerMacro getInstance() {
      if (instance == null) {
         instance = new SlayerMacro();
      }
      return instance;
   }
   ...
   public void invokeState() {
      ...
      switch (currentState) {
         ...
         case WALKING:
            ...
            if (MacroHandler.walker.isDone() && target == null) {
               ignoreEntity = new ArrayList<>();
            ...
            if (!visible && !killingBoss && !killingMiniBoss) {
               ...
               if (Config.SAttackType == Config.SAttackEnum.LEFT_CLICK.ordinal()) {
                  ...
                  if (killingBoss || killingMiniBoss) {
                     ...
                     normalMovement(3, mc.gameSettings.keyBindAttack);
                  }
               } else {
                  if (killingBoss || killingMiniBoss) {
                     ...
                     if (Minecraft.getMinecraft().thePlayer.getDistanceToEntity(target) <= 6) {
                        ...
                        if (Minecraft.getMinecraft().thePlayer.getDistanceToEntity(target) <= 5.5) {
                           if (canWalk("Back")) {
   ...
   private void bossMovement(int distance, KeyBinding Key) {
      if (Minecraft.getMinecraft().thePlayer.getDistanceToEntity(target) <= distance) {
         ...
         if (Minecraft.getMinecraft().thePlayer.getDistanceToEntity(target) <= distance - .5) {
            if (canWalk("Back")) {
               KeyBindUtils.setKeyBindState(mc.gameSettings.keyBindBack, true);
            }
            else if (canWalk("Left")) {
   ...
   private void normalMovement(int distance, KeyBinding Key) {
      if (Minecraft.getMinecraft().thePlayer.getDistanceToEntity(target) <= distance) {
         MacroHandler.walker.stop();
