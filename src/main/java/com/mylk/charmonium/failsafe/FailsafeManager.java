package com.mylk.charmonium.failsafe;
...
public class FailsafeManager {
    ...
    public void possibleDetection(Failsafe failsafe) {
        ...
        if (!chooseEmergencyDelay.isScheduled())
            chooseEmergencyDelay.schedule(Config.failsafeStopDelay + (long) (Math.random() * 500));
        ...
        if (!sendingFailsafeInfo) {
            ...
            Multithreading.schedule(() -> {
                ...
                if (Config.enableFailsafeSound && failsafe.shouldPlaySound()) {
                    AudioManager.getInstance().playSound();
                }

                if (Config.autoAltTab && failsafe.shouldAltTab()) {
                    FailsafeUtils.bringWindowToFront();
                }
                Multithreading.schedule(() -> {
                    if (Config.autoAltTab && failsafe.shouldAltTab() && !Display.isActive()) {
    ...
    public void onTickRestartMacro(TickEvent.ClientTickEvent event) {
        ...
        if (restartMacroAfterFailsafeDelay.passed()) {
            ...
            }, Config.alwaysTeleportToGarden ? 1_500 : 0, TimeUnit.MILLISECONDS);
        }
    }
    ...
    public void onRenderOverlay(RenderGameOverlayEvent.Post event) {
        ...
        if (chooseEmergencyDelay.isScheduled()) {
            ...
            textLines.add("§cPRESS §6" + Config.toggleMacro.getDisplay() + "§c TO DISABLE THE MACRO");
    ...
    public void restartMacroAfterDelay() {
        if (Config.enableRestartAfterFailSafe) {
            ...
            Multithreading.schedule(() -> {
                ...
                if (Config.enableRestartAfterFailSafe) {
                    if (Config.restartAfterFailSafeDelay > 0) {
                        LogUtils.sendDebug("[Failsafe] Restarting the macro in " + Config.restartAfterFailSafeDelay + " minutes.");
                        restartMacroAfterFailsafeDelay.schedule(Config.restartAfterFailSafeDelay * 1_000L * 60L);
                    } else {
