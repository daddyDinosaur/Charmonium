...
public class AutoSell implements IFeature {
    ...
    public void enable(boolean manually) {
        ...
        enabled = true;
        marketType = Config.autoSellMarketType ? MarketType.NPC : MarketType.BAZAAR;
        npcState = NPCState.NONE;
        bazaarState = BazaarState.NONE;
        sacksState = SacksState.NONE;
        delayClock.reset();
    ...
    public void onTickShouldEnable(TickEvent.ClientTickEvent event) {
        ...
        if (inventoryFilledClock.isScheduled() && inventoryFilledClock.passed()) {
            if (getInventoryFilledPercentage() >= Config.inventoryFullRatio / 100f) {
                start();
            } else {
                inventoryFilledClock.reset();
            ...
            if (getInventoryFilledPercentage() >= Config.inventoryFullRatio / 100f) {
                inventoryFilledClock.schedule((long) (Config.inventoryFullTime * 1000f));
        ...
        switch (sacksState) {
            ...
            case OPEN_MENU:
                if (mc.currentScreen == null) {
                    ...
                    pickedUpItems = false;
                    if (Config.autoSellSacksPlacement)
                ...
                switch (bazaarState) {
                    ...
                    case SELL_INV:
                        ...
                        if (sellInventoryNowLore.stream().anyMatch(s -> s.contains("You don't have anything to"))) {
                            if (Config.autoSellSacks && sellSacksSlot != -1) {
            ...
            case NPC:
                switch (npcState) {
                    case NONE:
                        ...
                        if (!shouldSell && Config.autoSellSacks && !emptySacks) {
                            setSacksState(SacksState.OPEN_MENU);
                            if (mc.currentScreen != null) {
                                PlayerUtils.closeScreen();
                            }
                    ...
                    case CLOSE_MENU:
                        if (mc.currentScreen == null) {
                            ...
                            if (Config.autoSellSacks && !emptySacks) {
                                setSacksState(SacksState.OPEN_MENU);
                            } else {
                                stop();
                            }
