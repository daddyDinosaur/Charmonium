...
import com.mylk.charmonium.util.helper.SignUtils;
...
public class AutoGodPot implements IFeature {
    ...
    private final int STUCK_DELAY = (int) (7_500 + Config.macroGuiDelay + Config.macroGuiDelayRandomness);
    ...
    public void onTickShouldEnable(TickEvent.ClientTickEvent event) {
        ...
        if (!GameStateHandler.getInstance().atProperIsland()) return;
        ...
        switch (ahState) {
            ...
            case OPEN_AH:
                if (mc.currentScreen != null) {
                    ...
                    delayClock.schedule(Config.getRandomGUIMacroDelay());
                    break;
                }
                if (rightClick) {
                    if (rotation.isRotating()) return;
                    long randomTime = Config.getRandomRotationTime();
    ...
    private void onGoingToAHState() {
        switch (goingToAHState) {
            ...
            case ROTATE_TO_AH_2:
                ...
                KeyBindUtils.stopMovement();
                long randomTime2 = Config.getRandomRotationTime();
    ...
    private void onBackpackState() {
        switch (backpackState) {
            ...
            case OPEN_STORAGE:
                ...
                if (Config.autoGodPotStorageType) {
                    mc.thePlayer.sendChatMessage("/ec " + Config.autoGodPotBackpackNumber);
                } else {
                    mc.thePlayer.sendChatMessage("/bp " + Config.autoGodPotBackpackNumber);
                }
    ...
    private void noGodPotInInventory() {
        ...
        shouldTpToGarden = true;
        if (Config.autoGodPotFromBits) {
            setGodPotMode(GodPotMode.FROM_BITS_SHOP);
        } else if (Config.autoGodPotFromAH && GameStateHandler.getInstance().getCookieBuffState() == GameStateHandler.BuffState.NOT_ACTIVE) {
    ...
    private void onBitsShop() {
        switch (bitsShopState) {
            ...
            case ROTATE_TO_BITS_SHOP:
                ...
                KeyBindUtils.stopMovement();
                long randomTime3 = Config.getRandomRotationTime();
