...
public class FailsafeUtils {
    ...
    public FailsafeUtils() {
        ...
        BufferedImage image;
        try {
            image = ImageIO.read(Objects.requireNonNull(getClass().getResource("/charmonium/char.png")));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        trayIcon = new TrayIcon(image, "Charmonium Failsafe Notification");
        trayIcon.setImageAutoSize(true);
        trayIcon.setToolTip("Charmonium Failsafe Notification");
    ...
    private void windows(String text, TrayIcon.MessageType type) {
        if (SystemTray.isSupported()) {
            try {
                trayIcon.displayMessage("Charmonium", text, type);
            } catch (Exception e) {
                e.printStackTrace();
            }
        } else {
    ...
    private void mac(String text) {
        ...
        processBuilder.command("osascript", "-e", "display notification \"" + text + "\" with title \"Charmonium\"");
        try {
    ...
    private void linux(String text) {
        ProcessBuilder processBuilder = new ProcessBuilder();
        processBuilder.command("notify-send", "-a", "Charmonium", text);
        try {
            processBuilder.start();
        } catch (IOException e) {
            e.printStackTrace();
