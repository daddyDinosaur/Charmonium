...
public class AudioManager {
    ...
    public void resetSound() {
        ...
        if (Config.maxOutMinecraftSounds) {
        ...
        if (!Config.failsafeSoundType) {
            ...
            Multithreading.schedule(() -> {
                try {
                    ...
                    switch (Config.failsafeSoundSelected) {
                        case 0:
                            File audioFile = new File(mc.mcDataDir.getAbsolutePath() + "/charmonium_sound.wav");
                            if (audioFile.exists() && audioFile.isFile())
                                inputStream = AudioSystem.getAudioInputStream(audioFile);
                            break;
                        case 1:
                            inputStream = AudioSystem.getAudioInputStream(getClass().getResource("/charmonium/sounds/staff_check_voice_notification.wav"));
                            break;
                        case 2:
                            inputStream = AudioSystem.getAudioInputStream(getClass().getResource("/charmonium/sounds/metal_pipe.wav"));
                            break;
                        case 3:
                            inputStream = AudioSystem.getAudioInputStream(getClass().getResource("/charmonium/sounds/AAAAAAAAAA.wav"));
                            break;
                        case 4:
                            inputStream = AudioSystem.getAudioInputStream(getClass().getResource("/charmonium/sounds/loud_buzz.wav"));
                            break;
                    }
                    if (inputStream == null) {
                    ...
                    float volumePercentage = Config.failsafeSoundVolume / 100f;
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        ...
        if (Config.failsafeSoundType) return;
        if (!minecraftSoundEnabled) return;
        ...
        switch (Config.failsafeMcSoundSelected) {
            case 0: {
