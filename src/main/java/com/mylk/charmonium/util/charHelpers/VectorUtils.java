...
public class VectorUtils {
    ...
    public static Vec3 getClosestHittableToNextBlock(BlockPos currentBlock, BlockPos nextBlock) {
        ...
        for (int x = 1; x < 5; ++x) {
            for (int y = 1; y < 5; ++y) {
                for (int z = 1; z < 5; ++z) {
                    Vec3 vec = new Vec3(currentBlock.getX() + x / 4.0 - 0.125,
    ...
    public static MovingObjectPosition fastRayTrace(Vec3 vec31, final Vec3 vec32, final boolean hitLiquid) {
        ...
        if (block.canCollideCheck(iblockstate, hitLiquid)) {
            final MovingObjectPosition movingobjectposition = block.collisionRayTrace((World)mc.theWorld, blockpos, vec31, vec32);
        ...
        while (k2-- >= 0) {
            ...
            final MovingObjectPosition movingobjectposition2 = block2.collisionRayTrace((World)mc.theWorld, blockpos, vec31, vec32);
