...
public class AOTVWaypointsPage extends Page {
    ...
    public AOTVWaypointsPage() {
        ...
        addNewList.setClickAction(() -> {
            ...
            Charmonium.aotvWaypoints.getRoutes().add(new AOTVWaypointsStructs.WaypointList("New Route", false, false, new ArrayList<>()));
            redrawRoutes();
        });
        importList.setClickAction(() -> {
            ...
            try {
                ...
                if (data.startsWith("#CharmoniumWaypoint#::")) {
                    data = data.replace("#CharmoniumWaypoint#::", "");
                    data = new String(Base64.getDecoder().decode(data));
                    AOTVWaypointsStructs.WaypointList list = Charmonium.gson.fromJson(data, AOTVWaypointsStructs.WaypointList.class);
                    if (list != null) {
                        list.enabled = false;
                    ...
                    data = sb.toString();
                    CategoryList categoryList = Charmonium.gson.fromJson(data, CategoryList.class);
                    ...
                    for (WaypointCategory category : categories) {
                        ...
                        Charmonium.aotvWaypoints.getRoutes().add(list);
                        Notifications.INSTANCE.send("Charmonium", "Imported route " + list.name + " from Skytils successfully!");
                    }
                    ...
                    data = new String(Base64.getDecoder().decode(data));
                    System.out.println(data);
                    JsonElement element = Charmonium.gson.fromJson(data, JsonElement.class);
                    if (element instanceof JsonObject) {
                        ...
                        if (object.has("routes")) {
                            ...
                            Notifications.INSTANCE.send("Charmonium", "Imported routes from Charmonium successfully!");
                            redrawRoutes();
                        }
                    } else if (element instanceof JsonArray) {
                        JsonArray array = (JsonArray) element;
                        ArrayList<Waypoint> waypoints = Charmonium.gson.fromJson(element, new TypeToken<ArrayList<Waypoint>>() {
                        }.getType());
                ...
                Notifications.INSTANCE.send("Charmonium", "Failed to import route!");
                Notifications.INSTANCE.send("Charmonium", "Error message: " + e.getMessage());
                Notifications.INSTANCE.send("Charmonium", "Click me to copy the message.", 5000, () -> {
                    Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
    ...
    public static void redrawRoutes() {
        ...
        for (AOTVWaypointsStructs.WaypointList list : Charmonium.aotvWaypoints.getRoutes()) {
            ...
            selected.setClickAction(() -> {
                Charmonium.aotvWaypoints.getRoutes().forEach(waypointList -> waypointList.enabled = false);
                Charmonium.aotvWaypoints.getRoutes().get(Charmonium.aotvWaypoints.getRoutes().indexOf(list)).enabled = true;
                redrawRoutes();
            });
            ...
            expandButton.setClickAction(() -> {
                if (Objects.equals(expandButton.getText(), "Expand")) {
                    Charmonium.aotvWaypoints.getRoutes().forEach(waypointList -> waypointList.showCoords = false);
                    Charmonium.aotvWaypoints.getRoutes().get(Charmonium.aotvWaypoints.getRoutes().indexOf(list)).showCoords = true;
                } else if (Objects.equals(expandButton.getText(), "Hide")) {
                    Charmonium.aotvWaypoints.getRoutes().get(Charmonium.aotvWaypoints.getRoutes().indexOf(list)).showCoords = false;
                }
                redrawRoutes();
            });
            ...
            BasicButton deleteButton = new BasicButton(80, BasicButton.SIZE_36, "Delete", null, null, BasicButton.ALIGNMENT_CENTER, ColorPalette.PRIMARY_DESTRUCTIVE);
            deleteButton.setClickAction(() -> {
                Charmonium.aotvWaypoints.getRoutes().remove(list);
                redrawRoutes();
            });
            ...
            shareButton.setClickAction(() -> {
                String routeString = Base64.getEncoder().encodeToString(Charmonium.gson.toJson(list).getBytes());
                StringSelection stringSelection = new StringSelection("#CharmoniumWaypoint#::" + routeString);
                Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
                clipboard.setContents(stringSelection, null);
            });
    ...
    public void draw(long vg, int x, int y, InputHandler inputHandler) {
        ...
        if (Charmonium.aotvWaypoints == null || Charmonium.aotvWaypoints.getRoutes().size() == 0 || routes.size() == 0) {
